function plot_bandit_gradient(game_config, rewards, choices, model_outputs)
    figure;
    
    
    n_arms = length(game_config.probs);
    n_trials = game_config.n_blocks * game_config.block_size;
    ...
    for block = 1:game_config.n_blocks
        start_idx = (block - 1) * game_config.block_size + 1;
        final_idx = block * game_config.block_size;

        game_probs(:, start_idx:final_idx) = repmat(arm_probs', 1, game_config.block_size);
        
        arm_probs = circshift(arm_probs, 1);
    end
